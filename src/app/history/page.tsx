...
export default function HistoryPage() {
  ...
  const filteredSessions = sessions
    .filter((session) => {
      ...
      const patientNames = session.patients
        .map((p) => p.name ?? '')
        .join(' ');
      return (
  ...
  const analyzedSessions = sessions.filter((s) =>
    s.patients.some((p) => p.summary)
  ).length;
      ...
      <div className="sticky top-0 z-40 bg-dark-bg border-b border-slate-700/50 shadow-lg">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-white font-bold text-lg">세션 기록</h1>
