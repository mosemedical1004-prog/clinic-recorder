...
import { getStoredApiKey, saveApiKey, deleteApiKey } from '@/lib/apiKey';
import {
...
export default function SettingsTab() {
  ...
  const [apiKeyStored, setApiKeyStored] = useState(() => !!getStoredApiKey());
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center flex-1 bg-dark-bg">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="bg-dark-card border border-slate-700/50 rounded-2xl overflow-hidden">
          ...
          <div className="p-5 space-y-5">
            <div>
              <label className="block text-slate-300 text-sm font-medium mb-2">음성 인식 언어</label>
              <select
              ...
              <p className="text-slate-500 text-xs mt-1">Chrome/Edge 브라우저에서 Web Speech API를 사용합니다</p>
            </div>
        ...
        <section className="bg-dark-card border border-slate-700/50 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-700/30">
            ...
            <p className="text-slate-500 text-xs mt-0.5">녹음 종료 시 선택한 폴더에 TXT 파일이 자동으로 저장됩니다</p>
          </div>
          ...
          <div className="p-5 space-y-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-slate-500 text-xs">키워드 또는 2분 침묵으로 새 환자를 자동으로 감지합니다</div>
              </div>
              <button
                onClick={() => setSettings((prev) => ({ ...prev, autoDetectPatients: !prev.autoDetectPatients }))}
                className={`relative w-12 h-6 rounded-full transition-colors ${settings.autoDetectPatients ? 'bg-blue-600' : 'bg-slate-600'}`}
              >
                <div className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform ${settings.autoDetectPatients ? 'translate-x-6' : 'translate-x-0'}`} />
              </button>
            </div>

            <div>
              <label className="block text-slate-300 text-sm font-medium mb-2">감지 키워드</label>
              <p className="text-slate-500 text-xs mb-3">아래 키워드가 음성에서 감지되면 새 환자 구간이 시작됩니다</p>
              <div className="flex flex-wrap gap-2 mb-3">
                {settings.patientKeywords.map((keyword) => (
                  <div key={keyword} className="flex items-center gap-1 px-2.5 py-1 bg-slate-800 border border-slate-600 rounded-full text-sm">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-slate-300 text-sm font-medium">침묵 감지 시간</label>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    min={1}
              ...
              <input
                ...
                step={1}
                value={settings.silenceTimeoutMinutes}
                onChange={(e) => setSettings((prev) => ({ ...prev, silenceTimeoutMinutes: Number(e.target.value) }))}
                className="w-full accent-blue-500"
              />
              <div className="flex justify-between text-slate-500 text-xs mt-1">
                <span>1분</span>
              ...
              <p className="text-slate-500 text-xs mt-2">마지막 발화 후 이 시간 이상 침묵하면 새 환자로 자동 감지합니다</p>
            </div>
          </div>
        </section>
          ...
          <div className="p-5">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative w-12 h-6 rounded-full transition-colors ${settings.darkMode ? 'bg-blue-600' : 'bg-slate-600'}`}
              >
                <div className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform ${settings.darkMode ? 'translate-x-6' : 'translate-x-0'}`} />
              </button>
            </div>
          </div>
        </section>
        ...
        <section className="bg-dark-card border border-slate-700/50 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-700/30">
            <div className="flex items-center justify-between">
              <h2 className="text-white font-semibold">AI 요약 설정</h2>
              <span className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${
                apiKeyStored
                  ? 'bg-green-900/30 text-green-400 border border-green-700/30'
                  : 'bg-slate-700/50 text-slate-500 border border-slate-600/30'
              }`}>
                <span className={`w-1.5 h-1.5 rounded-full ${apiKeyStored ? 'bg-green-500' : 'bg-slate-600'}`} />
                {apiKeyStored ? '등록됨' : '미등록'}
              </span>
            </div>
          </div>
          <div className="p-5 space-y-4">
            {/* Description */}
            <div className="bg-slate-800/40 rounded-xl p-4 space-y-1.5 text-xs text-slate-400 leading-relaxed">
              <p className="text-slate-300 font-medium text-sm mb-2">AI 요약 기능 안내</p>
              <p>AI 요약 기능은 Anthropic의 Claude API를 사용합니다.</p>
              <p>사용하려면 본인의 API Key가 필요합니다.</p>
              <ul className="space-y-1 mt-2 pl-1">
                <li>• API Key 발급: <span className="text-blue-400">https://console.anthropic.com</span></li>
                <li>• 가입 후 무료 크레딧 제공</li>
                <li>• 진료 1건 요약 시 약 0.01달러(약 10원) 소모</li>
                <li className="text-slate-500">• API Key는 본인 브라우저에만 저장되며 외부로 전송되지 않습니다</li>
              </ul>
            </div>
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <input
                    ...
                    placeholder={apiKeyStored ? '등록된 Key가 있습니다 (변경하려면 입력)' : 'sk-ant-...'}
                    className="w-full px-3 py-2.5 pr-10 bg-slate-800 border border-slate-600 rounded-xl text-white text-sm placeholder-slate-600 focus:outline-none focus:border-blue-500 font-mono"
                  />
                  <button
                    type="button"
            ...
            <div className="flex gap-2">
              ...
              {apiKeyStored && (
                <button
                  ...
                  className="px-4 py-2.5 bg-red-900/20 hover:bg-red-900/40 text-red-400 border border-red-800/30 rounded-xl text-sm font-medium transition-colors"
                >
                  삭제
                </button>
              )}
            ...
            <div className="flex items-center justify-between pt-1 border-t border-slate-700/30">
              <span className="text-slate-500 text-xs">사용 모델</span>
              <span className="text-slate-400 text-xs font-mono">claude-sonnet-4-6</span>
            </div>
          </div>
        </section>
        ...
        <section className="bg-dark-card border border-slate-700/50 rounded-2xl overflow-hidden">
          ...
          <div className="p-5">
            <div className="space-y-3">
              {[
                { name: '음성 인식 (Web Speech API)', check: () => typeof window !== 'undefined' && ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window), note: 'Chrome, Edge 권장' },
                { name: '마이크 접근 (MediaRecorder)', check: () => typeof window !== 'undefined' && 'MediaRecorder' in window, note: '모든 현대 브라우저' },
                { name: '화면 유지 (Wake Lock API)', check: () => typeof navigator !== 'undefined' && 'wakeLock' in navigator, note: 'Chrome, Edge' },
                { name: '로컬 저장 (IndexedDB)', check: () => typeof window !== 'undefined' && 'indexedDB' in window, note: '모든 현대 브라우저' },
              ].map((feature) => {
                const isSupported = typeof window !== 'undefined' ? feature.check() : false;
                return (
                  <div key={feature.name} className="flex items-center justify-between">
                    ...
                    <div className={`flex items-center gap-1.5 text-xs font-medium ${isSupported ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <div className="p-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-slate-500 text-xs">저장된 모든 세션과 설정을 영구적으로 삭제합니다</div>
              </div>
              <button
              ...
              >
                {clearing ? '삭제 중...' : clearConfirm ? '정말 삭제하시겠습니까?' : '전체 삭제'}
              </button>
            </div>
          </div>
        </section>
        ...
        <div className="text-center text-slate-600 text-xs pb-4">
          <p>진료 녹음기 v0.1.0</p>
          <p className="mt-1">Powered by Claude AI · Built with Next.js</p>
        </div>
      </div>
    </div>
  );
